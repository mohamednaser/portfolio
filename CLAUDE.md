...
- `src/content/blog/*.mdx` — posts. `src/content/projects`, `src/content/case-studies` likewise.
- `src/components/posts/*.astro` — long-form article bodies (raw HTML + inline SVG) imported by thin MDX wrappers. Use this when a post carries custom markup MDX would choke on.
- `src/pages/` — routes. `blog/[slug].astro` renders posts; `rss.xml.ts` builds the feed.
...
- **Scope custom post CSS** under a wrapper class (e.g. `.lynk-post`) in `<style is:global>`; never restyle `body`, `*`, or bare element selectors globally.
