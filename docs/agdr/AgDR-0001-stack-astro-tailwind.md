...
| **Plain static HTML + Tailwind + Vite** | Lowest dependency footprint; full control; no framework risk | No content model; 8+ pages means copy-paste boilerplate (nav, footer, head); blog index requires hand-rolled scripts; the case for ditching the current site is partly *because* it's hand-rolled HTML — this would recreate that |
